...
public class SynchronizationService {
    ...
    private static final Map<Integer, ApiEndpoint> API_ENDPOINTS = Map.of(
        8081, new ApiEndpoint("http://localhost:8081", Championship.PREMIER_LEAGUE),
        8082, new ApiEndpoint("http://localhost:8082", Championship.LA_LIGA)
    );
            ...
            try {
                ...
                if (clubs != null) {
                    ...
                    for (ClubDTO club : clubs) {
                        try {
                            List<PlayerDTO> clubPlayers = fetchClubPlayers(endpoint.getUrl(), club.getId(), endpoint.getChampionship());
                            if (clubPlayers != null) {
                                // We don't add these to allPlayers to avoid duplicates
                                // but we could process them if needed
                            }
                        } catch (RestClientException e) {
                            logger.error("Error fetching players for club {}: {}", club.getId(), e.getMessage());
                        }
                    }
                }
                ...
                // For each player, fetch statistics
                if (players != null) {
                    for (PlayerDTO player : players) {
                        try {
                            PlayerStatisticsDTO playerStats = fetchPlayerStatistics(endpoint.getUrl(), player.getId(), CURRENT_SEASON_YEAR);
                            // Process player statistics if needed
                        } catch (RestClientException e) {
                            logger.error("Error fetching statistics for player {}: {}", player.getId(), e.getMessage());
                        }
                    }
                }
